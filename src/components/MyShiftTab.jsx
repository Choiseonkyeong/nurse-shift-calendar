...
import { ChevronLeft, ChevronRight, Edit3, Trash2, Bell, Clock, Volume2, Check } from 'lucide-react';

export default function MyShiftTab({
  currentYear, currentMonth,
  selectedDate, setSelectedDate,
  handlePrevMonth, handleNextMonth,
  shiftConfigs, myShifts, memos, today,
  handleShiftChange, setMemos
}) {
  // 알림 및 메모 설정 상태
  ...
  const [alertOffset, setAlertOffset] = useState('5'); // 기본 5분 전
        ...
        <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200 space-y-2">
          <div className="flex justify-between items-center text-xs font-extrabold text-slate-800">
            <span className="flex items-center gap-1">
              ...
              <span>{selectedDate} 근무 빠른 변경</span>
            </span>
            <span className="text-indigo-600">{currentSelectedShiftCode ? shiftConfigs[currentSelectedShiftCode]?.name : '미등록'}</span>
          </div>
